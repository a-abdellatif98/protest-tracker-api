...
module.exports = {
  addressToGeocodedLocation: async function (fullAddress) {
    if (!geocodeCache[fullAddress]) {
      const geocoder = NodeGeocoder({provider: 'openstreetmap'});
      let geoAddr = (await geocoder.geocode(fullAddress))[0] || {};
      if (geoAddr.latitude && geoAddr.longitude) {
        geoAddr.timezone = geoTz(geoAddr.latitude, geoAddr.longitude)[0];
      } else {
        geoAddr.timezone = 'America/New_York';
      }
      geocodeCache[fullAddress] = geoAddr;
    }
